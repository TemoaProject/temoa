...
def _copy_tutorial_resources(target_config: Path, target_database: Path) -> None:
    ...
    The database is generated by applying the canonical v4 schema first,
    then loading tutorial data SQL.
    ...
    try:
        ...
        if not fallback_schema.exists():
            raise FileNotFoundError(
                f'Schema not found. Tried package resources and fallback path:\n'

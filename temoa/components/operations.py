...
def ramp_up_day_constraint(
    ...
    ramp_upSeason_constraint. ramp_upDay constrains ramp rates between time slices
    within each season and ramp_upSeason constrains ramp rates between sequential
    seasons. If the :code:`time_sequencing` parameter is set to :code:`consecutive_days`
    then the ramp_upSeason constraint is skipped as seasons already connect together.
    ...
    .. math::
       :label: ramp_upDay
            ...
            \\
            \forall \{r, p, s, d, t, v\} \in \Theta_{\text{ramp_upDay}}
            \\
...
def ramp_down_day_constraint(
    ...
    Similar to the :code`ramp_upDay` constraint, we use the :code:`ramp_downDay`
    ...
    .. math::
       :label: ramp_downDay
            ...
            R_{r,t} \cdot \Delta H_{r,p,s,d,s_{next},d_{next}} \cdot CAP_{r,p,t,v} \cdot C2A_{r,t}
            \\
            \forall \{r, p, s, d, t, v\} \in \Theta_{\text{ramp_downDay}}
    """
...
def ramp_up_season_constraint(
    ...
    of sequential seasons. Same as ramp_upDay but only applies to the boundary
...
def ramp_down_season_constraint(
    ...
    of sequential seasons. Same as ramp_downDay but only applies to the boundary
    between sequential seasons, i.e., :math:`(s^{seq},d_{last})` to :math:`(s^{seq}_{next},d_{first})`
    and :math:`s^{seq}_{next}` is based on the TimeSequential table rather than the
    time_season table.
    """

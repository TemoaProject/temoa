...
provided to the model without a preceding processing chain if none is available.
...
.. figure:: images/commodity_network.png
   :alt: Good Network
...
.. figure:: images/broken_commodity_network.png
   :alt: Bad Network
...
``Network Data Manager`` will preload essential data in order to analyze all networks (each region/period) within the
...
The approach described above is applied individually to each region-period pair within the optimization window.

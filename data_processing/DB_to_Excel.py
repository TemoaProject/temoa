...
def make_excel(ifile, ofile, scenario):
    ...
    query = (
        "SELECT regions, tech, sector, t_periods, capacity FROM Output_CapacityByPeriodAndTech WHERE scenario='"
        + scenario
        + "'"
    )
    ...
    for sector in sorted(df_capacity["sector"].unique()):
        ...
        df_capacity_sector = df_capacity_sector.drop(columns=["sector"]).pivot_table(
            values="capacity", index=["regions", "tech"], columns="t_periods"
        )
        df_capacity_sector.reset_index(inplace=True)
        sector_techs = all_techs[all_techs["sector"] == sector]
        df_capacity_sector = pd.merge(
            sector_techs[["regions", "tech"]],
            df_capacity_sector,
            on=["regions", "tech"],
            how="left",
        )
        df_capacity_sector.rename(
            columns={"regions": "Region", "tech": "Technology"}, inplace=True
        )
    ...
    query = (
        "SELECT regions, tech, sector, t_periods, sum(vflow_out) as vflow_out FROM Output_VFlow_Out WHERE scenario='"
        + scenario
        + "' GROUP BY \
    ...
    for sector in sorted(df_activity["sector"].unique()):
        ...
        df_activity_sector = df_activity_sector.drop(columns=["sector"]).pivot_table(
            values="vflow_out", index=["regions", "tech"], columns="t_periods"
        )
    ...
    query = (
        "SELECT regions, tech, sector, t_periods, emissions_comm, sum(emissions) as emissions FROM Output_Emissions WHERE scenario='"
        + scenario
        + "' GROUP BY \
    ...
    query = (
        "SELECT regions, tech, sector, output_name,  vintage, output_cost FROM Output_Costs WHERE output_name LIKE '%V_Discounted%' AND scenario='"
        + scenario
        + "'"
    )
    ...
    df_emissions_raw["variable"] = (
        "Emissions|"
        + df_emissions_raw["emissions_comm"]
        + "|"
        + df_emissions_raw["tech"]
    )
    df_emissions_raw.rename(
        columns={"t_periods": "year", "emissions": "value", "regions": "region"},
        inplace=True,
    )

    df_capacity["scenario"] = scenario
    df_capacity["unit"] = "?"
    df_capacity["variable"] = (
        "Capacity|" + df_capacity["sector"] + "|" + df_capacity["tech"]
    )
    df_capacity.rename(
        columns={"t_periods": "year", "capacity": "value", "regions": "region"},
        inplace=True,
    )

    df_activity["scenario"] = scenario
    df_activity["unit"] = "?"
    df_activity["variable"] = (
        "Activity|" + df_activity["sector"] + "|" + df_activity["tech"]
    )
    df_activity.rename(
        columns={"t_periods": "year", "vflow_out": "value", "regions": "region"},
        inplace=True,
    )
...
if __name__ == "__main__":
    try:
        ...
        opts, args = getopt.getopt(
            argv, "hi:o:s:", ["help", "input=", "output=", "scenario="]
        )

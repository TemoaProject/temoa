...
class DatabaseUtil(object):
    ...
    def getCommoditiesAndTech(self, inp_comm, inp_tech, region):
        ...
        if region == None:
            ...
            self.cur.execute(
                "SELECT input_comm, tech, output_comm FROM Efficiency WHERE regions LIKE '%"
                + region
                + "%' and (input_comm is "
                + inp_comm
                + " or output_comm is "
        ...
        return pd.DataFrame(
            self.cur.fetchall(), columns=["input_comm", "tech", "output_comm"]
        )
    ...
    def getExistingTechnologiesForCommodity(self, comm, region, comm_type="input"):
        ...
        if comm_type == "input":
            ...
            query = (
                "SELECT DISTINCT tech FROM Efficiency WHERE output_comm is '"
                + comm
                + "'"
            )
        if region:
    ...
    def getCapacityForTechAndPeriod(self, tech=None, period=None, region=None):
        ...
        query += (
            " FROM Output_CapacityByPeriodAndTech WHERE scenario == '"
            + self.scenario
            + "'"
        )
    ...
    def getEmissionsActivityForPeriod(self, period, region):
        ...
        if self.scenario is None or self.scenario == "":
            raise ValueError("For Output related queries, please set a scenario first")
        query = (
            "SELECT E.emis_comm, E.tech, SUM(E.emis_act*O.vflow_out) FROM EmissionActivity E, Output_VFlow_Out O "
        ...
        self.cur.execute(query)
        result = pd.DataFrame(
            self.cur.fetchall(), columns=["emis_comm", "tech", "emis_activity"]
        )
        return result
    ...
    def getCommodityWiseInputAndOutputFlow(self, tech, period, region):
        ...
        query = (
            "SELECT OF.input_comm, OF.output_comm, OF.vintage, OF.regions,\

...
class GraphvizDiagramGenerator(object):
    ...
    def CreateMainResultsDiagram(self, period, region, outputFormat="svg"):
        ...
        commodity_fmt = (
            "href=\"#\", onclick=\"loadNextGraphvizGraph('results', '%s', '%s')\""
        )
        flow_fmt = 'label="%.2f"'
        ...
        for i in range(len(V_Cap2)):
            ...
            etechs.add(
                (
                    row["tech"],
                    tech_attr_fmt % (row["tech"], row["capacity"], row["tech"], period),
                )
            )
        ...
        for i in range(len(EI2)):
            ...
            if row["input_comm"] != "ethos":
                ...
                ecarriers.add(
                    (row["input_comm"], commodity_fmt % (row["input_comm"], period))
                )
        ...
        for i in range(len(EO2)):
            ...
            ecarriers.add(
                (row["output_comm"], commodity_fmt % (row["output_comm"], period))
            )
        ...
        for i in range(len(EmiO2)):
            ...
            if row["emis_activity"] >= epsilon:
                eflowso.add(
                    (row["tech"], row["emis_comm"], flow_fmt % row["emis_activity"])
                )
    ...
    def CreateTechResultsDiagrams(
        self, period, region, tech, outputFormat="svg"
    ):  # tech results
        self.__log__(
        ...
        for i in range(len(flows)):
            ...
            if row["input_comm"] != "ethos":
                enodes.add(
                    (row["input_comm"], enode_attr_fmt % (row["input_comm"], period))
                )
                iedges.add((row["input_comm"], vnode, 'label="%.2f"' % row["flow_in"]))
            enodes.add(
                (row["output_comm"], enode_attr_fmt % (row["output_comm"], period))
            )
    ...
    def CreateCommodityPartialResults(self, period, region, comm, outputFormat="svg"):
        ...
        input_total = set(
            self.dbUtil.getExistingTechnologiesForCommodity(comm, region, "output")[
                "tech"
            ]
        )
        output_total = set(
            self.dbUtil.getExistingTechnologiesForCommodity(comm, region, "input")[
                "tech"
            ]
        )
        ...
        rc_node_fmt = (
            'color="%s", href="%s", shape="circle", fillcolor="%s", fontcolor="black"'
        )
        ...
        rcnode = (
            (
                comm,
                rc_node_fmt
                % (self.colors["commodity_color"], url, self.colors["fill_color"]),
            ),
        )
    ...
    def createCompleteInputGraph(
        self, region, inp_tech=None, inp_comm=None, outputFormat="svg"
    ):
        self.__log__(
...
if __name__ == "__main__":
    ...
    graphGen = GraphvizDiagramGenerator(
        input["ifile"], input["scenario_name"], input["region"]
    )
    graphGen.connect()
    graphGen.setGraphicOptions(
        greyFlag=input["grey_flag"], splinevar=input["splinevar"]
    )

...
class TemoaSolver(object):
    ...
    def temoa_setup(self):
        ...
        if self.config_filename == "":  # Called from the command line
            ...
            if config_flag == 1:  # Option 2 (using config file)
                self.options.path_to_lp_files = (
                    self.options.path_to_logs + sep + "lp_files"
                )
    ...
    def temoa_checks(self):
        ...
        if self.optimizer:
            ...
            if SE.isatty():
                ...
                if (
                    os.path.join("temoa_model", "config_sample_myopic")
    ...
    def solveWithMGA(self):
        ...
        temoaInstance1 = TemoaSolverInstance(
        ...
        temoaInstance1.instance.FirstObj = Objective(
            rule=TotalCost_rule, sense=minimize
        )
        ...
        while self.options.next_mga():
            ...
            try:
                txt_file_mga = open(
                    self.options.path_to_logs + os.sep + "Complete_OutputLog.log", "w"
                )
            ...
            if self.optimizer:
                if self.options.neos:
                    self.result = self.optimizer.solve(
                        self.instance, opt=self.options.solver
                    )
                else:
                    self.result = self.optimizer.solve(
                        self.instance,
                ...
                formatted_results = pformat_results(
                    self.instance, self.result, self.options
                )
                yield "\t[%8.2f]\n" % duration()
                SE.write("\r[%8.2f\n" % duration())
                self.txt_file.write("[%8.2f]\n" % duration())

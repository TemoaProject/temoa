...
class TemoaModel(AbstractModel):
    def __init__(self, *args, **kwds):
        ...
        self.commodityDStreamProcess = (
            dict()
        )  # The downstream process of a commodity during a period
        self.commodityUStreamProcess = (
            dict()
...
def validate_time(M):
    ...
    for year in M.time_exist:
        ...
        msg = (
            'Set "time_exist" requires integer-only elements.\n\n  Invalid '
            'element: "{}"'
        )
    ...
    for year in M.time_future:
        ...
        msg = (
            'Set "time_future" requires integer-only elements.\n\n  Invalid '
            'element: "{}"'
        )
        raise Exception(msg.format(year))
    ...
    if len(M.time_future) < 2:
        msg = (
            'Set "time_future" needs at least 2 specified years.  Temoa '
...
def CreateDemands(M):
    ...
    unset_distributions = set(
        cross_product(
            M.regions, M.time_season, M.time_of_day, unset_demand_distributions
        )
    )
...
def CreateRegionalIndices(M):
    ...
    for r_i in M.regions:
        if "-" in r_i:
            raise Exception(
                "Individual region names can not have '-' in their names: " + str(r_i)
            )
...
def CreateSparseDicts(M):
        ...
        for p in M.time_optimize:
            ...
            if t in M.tech_exchange and (r[: r.find("-")], p, i) not in M.exportRegions:
                M.exportRegions[
                    ...
                    set()
                )  # since t is in M.tech_exchange, r here has *-* format (e.g. 'US-Mexico').
                # r[:r.find("-")] extracts the region index before the "-".
            if (
                t in M.tech_exchange
                and (r[r.find("-") + 1 :], p, o) not in M.importRegions
            ):
            ...
            if t in M.tech_exchange:
                M.exportRegions[r[: r.find("-")], p, i].add(
                    (r[r.find("-") + 1 :], t, v, o)
                )
            if t in M.tech_exchange:
                M.importRegions[r[r.find("-") + 1 :], p, o].add(
                    (r[: r.find("-")], t, v, i)
                )
    ...
    M.activeCapacityAvailable_rpt = set(
        (r, p, t) for r, p, t in M.processVintages.keys() if M.processVintages[r, p, t]
    )
...
def CapacityFactorTechIndices(M):
    indices = set((r, s, d, t) for r, s, d, t, v in M.CapacityFactor_rsdtv)
...
"""
    ...
    indices = set(
        (r, t, v) for r, i, t, v, o in M.Efficiency.sparse_iterkeys() if v >= min_period
    )
...
def CapacityAnnualConstraintIndices(M):
    capacity_indices = set(
        (r, p, t, v) for r, p, t, v in M.activeActivity_rptv if t in M.tech_annual
    )

...
def pformat_results(pyomo_instance, pyomo_result, options):
    ...
    optimal_solutions = ("feasible", "globallyOptimal", "locallyOptimal", "optimal")
    ...
    def collect_result_data(cgroup, clist, epsilon):
        ...
        for name, data in cgroup.items():
            if "Value" not in data.keys() or (abs(data["Value"]) < epsilon):
                continue
    ...
    if (
        hasattr(options, "file_location")
        and os.path.join("temoa_model", "config_sample_myopic") in options.file_location
    ):
    ...
    for r, p, i, t, v, o in m.V_FlowOutAnnual:
        for s in m.time_season:
            for d in m.time_of_day:
                ...
                for e in emissions:
                    evalue = val_out * m.EmissionActivity[r, e, i, t, v, o]
                    svars["V_EmissionActivityByPeriodAndProcess"][
                        r, p, e, t, v
                    ] += evalue
    ...
    for r, p, s, d, i, t, v, o in m.V_Curtailment:
        ...
        svars["V_FlowIn"][r, p, s, d, i, t, v, o] = (
            val + value(m.V_FlowOut[r, p, s, d, i, t, v, o])
        ) / value(m.Efficiency[r, i, t, v, o])
    ...
    if (
        hasattr(options, "file_location")
        and os.path.join("temoa_model", "config_sample_myopic")
    ...
    if (
        ...
        for r, t, v in m.CostInvest.sparse_iterkeys():  # Returns only non-zero values
            ...
            icost *= (
                value(LLN[r, t, v])
                if not GDR
                else (x ** (P_0 - v + 1) * (1 - x ** (-value(LLN[r, t, v]))) / GDR)
            )
        ...
        for r, p, t, v in m.CostFixed.sparse_iterkeys():
            ...
            svars["Costs"][
                "V_UndiscountedFixedCostsByProcess", r, t, v
            ] += fcost * value(MPL[r, p, t, v])
        ...
        for r, p, t, v in m.CostVariable.sparse_iterkeys():
            ...
            svars["Costs"][
                "V_UndiscountedVariableCostsByProcess", r, t, v
            ] += vcost * value(MPL[r, p, t, v])
            vcost *= (
                value(MPL[r, p, t, v])
                if not GDR
                else (x ** (P_0 - p + 1) * (1 - x ** (-value(MPL[r, p, t, v]))) / GDR)
        ...
        for i in m.RegionalExchangeCapacityConstraint_rrtv.iterkeys():
            ...
            try:
                act_dir1 = value(
                    sum(
                        m.V_FlowOut[reg_dir1, p, s, d, S_i, tech, vintage, S_o]
                        ...
                        for S_o in m.ProcessOutputsByInput[
                            reg_dir1, p, tech, vintage, S_i
                        ]
                    )
                )
                act_dir2 = value(
                    sum(
                        m.V_FlowOut[reg_dir2, p, s, d, S_i, tech, vintage, S_o]
                        for p in m.time_optimize
                        if (
                            p
                            < vintage
                ...
                act_dir1 = value(
                    sum(
                        m.V_FlowOutAnnual[reg_dir1, p, S_i, tech, vintage, S_o]
                        for p in m.time_optimize
                        if (
                            p
                            < vintage
                ...
                act_dir2 = value(
                    sum(
                        m.V_FlowOutAnnual[reg_dir2, p, S_i, tech, vintage, S_o]
                        for p in m.time_optimize
                        if (
                            p
                            < vintage
            ...
            for item in list(svars["Costs"]):
                if item[2] == tech:
                    opposite_dir = (
                        item[1][item[1].find("-") + 1 :]
                        + "-"
                        + item[1][: item[1].find("-")]
                    )
                    if (item[0], opposite_dir, item[2], item[3]) in svars[
                        "Costs"
                    ].keys():
                    ...
                    if item[1] == reg_dir1:
                        if (act_dir1 + act_dir2) > 0:
                            ...
                            svars["Costs"][item] = (
                                svars["Costs"][item] * act_dir1 / (act_dir1 + act_dir2)
                            )
        ...
        for item in list(svars["Costs"]):
            if item[2] in m.tech_exchange:
                svars["Costs"][
                    (item[0], item[1][item[1].find("-") + 1 :], item[2], item[3])
                ] = svars["Costs"][item]
    ...
    if isinstance(options, TemoaConfig):
        ...
        if (
            ...
            if (
                does_input_file_table_exist
            ):  # This block distinguishes normal database from schema.
                # This is schema file.
            ...
            for i in options.dot_dat:
                cur.execute("DELETE FROM input_file WHERE id=1;")
                cur.execute(
                    "INSERT INTO input_file(id, file) VALUES(?, ?);", (1, '"' + i + '"')
                )
                break
            dat_to_db(i, con)
        ...
        if options.saveDUALS:
            if len(duals) != 0:
                overwrite_keys = [
                    str(tuple(x))
                    for x in duals.reset_index()[
                        ["constraint_name", "scenario"]
                    ].to_records(index=False)
                ]
                # delete records that will be overwritten by new duals dataframe
                cur.execute(
                    "DELETE FROM Output_Duals WHERE (constraint_name, scenario) IN (VALUES "
                    + ",".join(overwrite_keys)
                    + ")"
                )
        ...
        if options.saveEXCEL or options.saveTEXTFILE or options.keepPyomoLP:
            ...
            if options.saveEXCEL:
                ...
                make_excel(
                    options.output, new_dir + os.sep + options.scenario, temp_scenario
                )
...
def dat_to_db(input_file, output_schema, run_partial=False):
    ...
    for i in parsed_data["time_exist"]:
        ...
        output_schema.execute(
            "INSERT OR REPLACE INTO time_periods VALUES(" + i + ", 'e');"
        )
    for i in parsed_data["time_future"]:
        if i is "":
            continue
        output_schema.execute(
            "INSERT OR REPLACE INTO time_periods VALUES(" + i + ", 'f');"
        )
    ...
    for i in parsed_data["commodity_demand"]:
        if i is "":
            continue
        output_schema.execute(
            "INSERT OR REPLACE INTO commodities VALUES('" + i + "', 'd', '');"
        )
    for i in parsed_data["commodity_physical"]:
        if i is "":
            continue
        output_schema.execute(
            "INSERT OR REPLACE INTO commodities VALUES('" + i + "', 'p', '');"
        )
    for i in parsed_data["commodity_emissions"]:
        if i is "":
            continue
        output_schema.execute(
            "INSERT OR REPLACE INTO commodities VALUES('" + i + "', 'e', '');"
        )
    ...
    for i in parsed_data["ExistingCapacity"]:
        ...
        row_data.append("")
        output_schema.execute(
            "INSERT OR REPLACE INTO ExistingCapacity VALUES(?, ?, ?, ?, ?);", row_data
        )
    ...
    for i in parsed_data["Efficiency"]:
        ...
        row_data.append("")
        output_schema.execute(
            "INSERT OR REPLACE INTO Efficiency VALUES(?, ?, ?, ?, ?, ?);", row_data
        )
    ...
    for i in parsed_data["LifetimeTech"]:
        ...
        row_data.append("")
        output_schema.execute(
            "INSERT OR REPLACE INTO LifetimeTech VALUES(?, ?, ?);", row_data
        )
    ...
    for i in parsed_data["LifetimeProcess"]:
        ...
        row_data.append("")
        output_schema.execute(
            "INSERT OR REPLACE INTO LifetimeProcess VALUES(?, ?, ?, ?);", row_data
        )
    ...
    for i in parsed_data["EmissionActivity"]:
        ...
        if len(row_data) is 7:
            row_data.append("")
        output_schema.execute(
            "INSERT OR REPLACE INTO EmissionActivity VALUES(?, ?, ?, ?, ?, ?, ?, ?);",
            row_data,
        )

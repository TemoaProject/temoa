...
def db_2_dat(ifile, ofile, options):
    ...
    def query_table(t_properties, f):
        ...
        t_index = t_properties[
            4
        ]  # table column index after which '#' should be specified
        if (
            type(t_flag) is list
        ):  # tech production table has a list for flags; this is currently hard-wired
            db_query = (
                "SELECT * FROM "
                + t_name
                + " WHERE flag=='p' OR flag=='pb' OR flag=='ps'"
            )
        ...
        cur.execute(db_query)
        if t_index == 0:
            # make sure that units and descriptions are commented out in DAT file
            for line in cur:
                str_row = str(line[0]) + "\n"
                f.write(str_row)
                print(str_row)
            ...
            for line in cur:
                ...
                after_comments = re.sub("[',)]", "    ", str(after_comments))
                search_afcom = re.search(
                    r"^\W+$", str(after_comments)
                )  # Search if after_comments is empty.
                if not search_afcom:
    ...
    with open(ofile, "w") as f:
        ...
        if options.myopic:
            ...
            cur.execute(
                "DELETE FROM Output_Costs WHERE scenario="
                + "'"
                + str(options.scenario)
                + "'"
            )
            cur.execute(
                "DELETE FROM Output_Objective WHERE scenario="
                + "'"
                + str(options.scenario)
                + "'"
            )
            ...
            cur.execute(
                "DELETE FROM Output_Duals WHERE scenario="
                + "'"
                + str(options.scenario)
                + "'"
            )
...
class TemoaConfig(object):
    ...
    tokens = (
        ...
        "keep_pyomo_lp_file",
        "saveEXCEL",
        "myopic" "myopic_periods" "keep_myopic_databases" "saveDUALS" "saveTEXTFILE",
        "mgaslack",
        "mgaiter",
        "path_to_data",
        "path_to_logs",
    ...
    def __init__(self, **kwargs):
        ...
        self.path_to_data = re.sub(
            "temoa_model$", "data_files", dirname(abspath(__file__))
        )  # Path to where automated excel and text log folder will be save as output.
        self.path_to_logs = (
            self.path_to_data + sep + "debug_logs"
        )  # Path to where debug logs will be generated for each run. By default in debug_logs folder in db_io.
    ...
    def __repr__(self):
        ...
        msg += "{:>{}s}: {}\n".format(
            "Retain myopic databases", width, self.KeepMyopicDBs
        )
        msg += spacer
        ...
        msg += "{:>{}s}: {}\n".format(
            "Solver LP write status", width, self.generateSolverLP
        )
    ...
    def t_ANY_error(self, t):
        if not self.__error:
            self.__error.append(
                {
                    "line": [t.lineno, t.lineno],
                    "index": [t.lexpos, t.lexpos],
                    "value": t.value[0],
                }
            )
    ...
    def build(self, **kwargs):
        ...
        db_or_dat = True
        # True means input file is a db file. False means input is a dat file.
        ...
        if self.__error:
            ...
            try:
                txt_file = open(
                    self.path_to_logs + os.sep + "Complete_OutputLog.log", "w"
                )

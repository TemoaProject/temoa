...
def myopic_db_generator_solver(self):
    ...
    table_list = cur_org.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'Output%'"
    ).fetchall()
    time_periods = cur_org.execute(
        "SELECT t_periods FROM time_periods WHERE flag='f'"
    ).fetchall()
    cur_org.execute("DELETE FROM MyopicBaseyear")
    cur_org.execute(
        "INSERT INTO MyopicBaseyear (year) VALUES (" + str(time_periods[0][0]) + ")"
    )
    con_org.commit()
    loc1 = max(
        loc for loc, val in enumerate(self.options.output) if val == "/" or val == "\\"
    )
    loc2 = max(loc for loc, val in enumerate(self.options.output) if val == ".")
    db_name = self.options.output[loc1 + 1 : loc2]
    copyfile(
        db_path_org,
        os.path.join(self.options.path_to_data, db_name)
        + "_blank"
        + self.options.output[loc2:],
    )
    ...
    if 1 <= int(N) <= len(time_periods) - 2:
        ...
        print(
            "Error: The number of myopic years must between 1 and "
            + str(len(time_periods) - 2)
        )

    for i in range(N - 1, len(time_periods) - 1):
        print(
            "Preparing the database for the period(s): "
            + str([str(time_periods[j][0]) for j in range(i - (N - 1), i + 1)])
        )
        ...
        # ---------------------------------------------------------------
        cur.execute(
            "DELETE FROM Efficiency WHERE vintage > " + str(time_periods[i][0]) + ";"
        )

        cur.execute(
            "UPDATE Efficiency SET tech = TRIM(tech);"
        )  # trim spaces. Need to trim carriage return

        # Delete row from Efficiency if (t,v) retires at the begining of current period (which is time_periods[i][0])
        cur.execute(
            "DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM LifetimeProcess WHERE \
                     LifetimeProcess.life_process+LifetimeProcess.vintage<="
            + str(time_periods[i - (N - 1)][0])
            + ") \
                     AND vintage IN (SELECT vintage FROM LifetimeProcess WHERE LifetimeProcess.life_process+\
                     LifetimeProcess.vintage<="
            + str(time_periods[i - (N - 1)][0])
            + ");"
        )
        ...
        WHERE Efficiency.vintage + LifetimeTech.life <= "
            + str(time_periods[i - (N - 1)][0])
            + ")\
        ...
        cur.execute(
            "DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM Efficiency WHERE \
                    40+Efficiency.vintage<="
            + str(time_periods[i - (N - 1)][0])
            + ") AND \
        ...
        while (
            len(
                cur.execute(
                    "SELECT * FROM Efficiency WHERE output_comm NOT IN (SELECT input_comm FROM Efficiency)\
            ...
            cur.execute(
                "DELETE FROM Efficiency WHERE output_comm NOT IN (SELECT input_comm FROM Efficiency) \
                     AND output_comm NOT IN (SELECT comm_name FROM commodities WHERE flag='d');"
            )
            iterval += 1
            if iterval > 10:
        ...
        cur.execute(
            "DELETE FROM time_periods WHERE t_periods > "
            + str(time_periods[i][0])
            + ";"
        )
            ...
            if i != (N - 1):
                ...
                for ind, row in df_linkedtechs.iterrows():
                    primary_tech = pd.read_sql_query(
                        "SELECT primary_tech FROM LinkedTechs WHERE primary_region = '"
                        + row["regions"]
                        + "' AND linked_tech = '"
                        + row["tech"]
                        + "'",
                    ...
                    df_primary_tech = pd.read_sql_query(
                        "SELECT * FROM "
                        + table
                        + " \
                                                             WHERE scenario="
                ...
                df_primarytechs = pd.read_sql_query(
                    "SELECT * FROM "
                    + table
                    + "  \
                                                             WHERE scenario="
                ...
                for ind, row in df_primarytechs.iterrows():
                    linked_tech = pd.read_sql_query(
                        "SELECT linked_tech FROM LinkedTechs WHERE primary_region = '"
                        + row["regions"]
                        + "' AND primary_tech = '"
                        + row["tech"]
                        + "'",
                    ...
                    df_linked_tech = pd.read_sql_query(
                        "SELECT * FROM "
                        + table
                        + " \
                                                             WHERE scenario="
        ...
        if i != (N - 1):
            df_new_ExistingCapacity = pd.read_sql_query(
                "SELECT regions, tech, vintage, capacity FROM Output_V_Capacity \
                                                         WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND \
            ...
            df_new_ExistingCapacity.columns = [
                "regions",
                "tech",
                "vintage",
                "exist_cap",
            ]
            df_new_ExistingCapacity.to_sql(
                "ExistingCapacity", con, if_exists="append", index=False
            )
            ...
            for j in range(N - 1, -1, -1):  # backward loop
                cur.execute(
                    "INSERT INTO Efficiency (regions,input_comm,tech,vintage,output_comm,efficiency) \
                             SELECT DISTINCT regions,input_comm,tech,"
                    + str(time_periods[i - j][0])
                    + ",output_comm,efficiency FROM Efficiency WHERE tech NOT IN (SELECT tech \
                             FROM Efficiency WHERE vintage < "
                    + str(time_periods[0][0])
                    + ") AND tech NOT IN (SELECT \
                             tech FROM Efficiency WHERE vintage >= "
                    + str(time_periods[i - j][0])
                    + ");"
                )
            ...
            cur.execute(
                "DELETE FROM Efficiency \
                         WHERE vintage <= "
                + str(time_periods[i - N][0])
                + " AND vintage NOT IN (SELECT \
            ...
            cur_org.execute(
                "DELETE FROM Output_CapacityByPeriodAndTech WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND t_periods>"
                + str(time_periods[i - N][0])
            ...
            cur_org.execute(
                "DELETE FROM Output_Emissions WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND t_periods>"
                + str(time_periods[i - N][0])
            ...
            cur_org.execute(
                "DELETE FROM Output_VFlow_In WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND t_periods>"
                + str(time_periods[i - N][0])
            ...
            cur_org.execute(
                "DELETE FROM Output_VFlow_Out WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND t_periods>"
                + str(time_periods[i - N][0])
            ...
            cur_org.execute(
                "DELETE FROM Output_V_Capacity WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND vintage>"
                + str(time_periods[i - N][0])
            ...
            cur_org.execute(
                "DELETE FROM Output_Curtailment WHERE scenario="
                + "'"
                + str(self.options.scenario)
                + "' AND t_periods>"
                + str(time_periods[i - N][0])
        ...
        for table in table_list:
            ...
            try:
                ...
                cur.execute(
                    "UPDATE " + str(table[0]) + " SET tech = TRIM(tech, CHAR(37,10));"
                )
                ...
                cur.execute(
                    "DELETE FROM "
                    + str(table[0])
                    + " WHERE tech NOT IN (SELECT tech FROM Efficiency);"
                )
                ...
                if "regions" in names:
                    query = (
                        "DELETE FROM "
                        + str(table[0])
                        + " WHERE (regions, tech) NOT IN (SELECT DISTINCT regions, tech FROM Efficiency) \
                    AND regions!='global'"
                    )
                    cur.execute(query)
                    query = (
                ...
                if "vintage" in names:
                    if table[0] != "ExistingCapacity":
                        for j in range(N - 1, -1, -1):
                            names = list(map(lambda x: x[0], cursor.description))
                            names = [
                                str(time_periods[i - j][0]) if x == "vintage" else x
                                for x in names
                            ]
                            query = (
                                "SELECT DISTINCT "
                                ...
                                + " FROM "
                                + table[0]
                                + " WHERE tech NOT IN (SELECT tech FROM "
                                + table[0]
                                + " WHERE vintage<"
                                + str(time_periods[0][0])
                                + ") AND tech NOT IN (SELECT tech FROM "
                                + table[0]
                                + " WHERE vintage >= "
                                + str(time_periods[i - j][0])
                                + ");"
                            ...
                            if table[0] == "EmissionActivity":
                                filter_list = names[
                                    : names.index(str(time_periods[i - j][0])) + 2
                                ]
                            else:
                                filter_list = names[
                                    : names.index(str(time_periods[i - j][0])) + 1
                                ]
                            df_table = df_table.drop_duplicates(
                                subset=filter_list, keep="last"
                            )
                            df_table.columns = [
                                "vintage" if x == str(time_periods[i - j][0]) else x
                                for x in df_table.columns
                            ]
                            df_table.to_sql(
                                str(table[0]), con, if_exists="append", index=False
                            )
                    ...
                    if table[0] == "CostVariable" or table[0] == "CostFixed":
                        cur.execute(
                            "DELETE FROM " + str(table[0]) + " WHERE periods < vintage;"
                        )
                    ...
                    if table[0] == "EmissionActivity":
                        cur.execute(
                            "DELETE FROM EmissionActivity WHERE regions || input_comm || tech || vintage || output_comm \
                        ...
                        cur.execute(
                            "DELETE FROM "
                            + str(table[0])
                            + " WHERE tech IN (SELECT tech FROM Efficiency) AND vintage \
                                 NOT IN (SELECT vintage FROM Efficiency WHERE Efficiency.tech="
                            + str(table[0])
                            + ".tech \
                                 AND Efficiency.regions="
                            + str(table[0])
                            + ".regions);"
                        )
            # except:
        ...
        cur.execute(
            "UPDATE commodities SET comm_name = TRIM(comm_name, CHAR(10,13,37))"
        )
        ...
        cur.execute(
            "INSERT INTO `time_periods` (t_periods,flag) VALUES ("
            + str(time_periods[i + 1][0])
            + ",'f');"
        )
        cur.execute(
            "UPDATE `time_periods` SET flag='e' WHERE t_periods < "
            + str(time_periods[i - (N - 1)][0])
        )
        ...
        if (i != (N - 1)) & ("MaxResource" in [x[0] for x in table_list]):
            resource_constraints_org = pd.read_sql_query(
                "SELECT regions, tech, maxres FROM MaxResource", con_org
            )
            for ind, row in resource_constraints_org.iterrows():
                df_existing_resources = pd.read_sql_query(
                    "SELECT sum(vflow_out) FROM Output_VFlow_Out \
                                                        WHERE regions='"
                    + row["regions"]
                    + "' AND \
                                                        tech='"
                ...
                try:
                    ...
                    query = (
                        "UPDATE MaxResource SET maxres="
                        + str(updated_resource)
                        + " WHERE regions='"
                        + row["regions"]
                        + "' AND tech='"
                        + row["tech"]
        ...
        # ---------------------------------------------------------------
        new_config = (
            os.path.join(os.getcwd(), "temoa_model", "config_sample") + new_myopic_name
        )
        if version < 3:
            ifile = io.open(
                os.path.join(os.getcwd(), "temoa_model", "config_sample"),
                encoding="utf-8",
            )
        else:
            ifile = open(
        ...
        for line in ifile:
            new_line = line.replace(
                "--input=data_files/" + db_name,
                "--input=data_files/" + db_name + new_myopic_name,
            )
        ...
        ofile.close()
        os.system(
            "python temoa_model/ --config=temoa_model/config_sample" + new_myopic_name
        )
